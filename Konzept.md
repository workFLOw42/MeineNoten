...
└── java/com/example/meinenoten/
    ...
    └── ui/
        ...
        │   └── SetlistStrip.kt    (neu) Sprungleiste mit Vorschaubildern
        ├── util/
        │   ├── SortUtils.kt       (neu) Collator-Sortierung
        │   └── DateUtils.kt       (neu) Datum deuten und anzeigen
        └── screens/
            ├── SongListScreen.kt
            ├── SongDetailScreen.kt    Anzeige + Eingabekonzept
            ├── SetlistScreen.kt
            └── SetlistDetailScreen.kt
```

Mit `(neu)` markierte Dateien entstehen im Rahmen der unter *In Arbeit* genannten
Änderungen.
